...
const Navbar = ({ toggle }) => {
  ...
  const toggleHome = () => {
    ...
    return (
        <nav className={scrollNav === true ? "z-20 fixed top-0 left-0 right-0 mr-20 ml-20 mt-5 flex items-center justify-between flex-wrap bg-teal-400 p-6 ease-out duration-300 bg-opacity-80 backdrop-blur-lg rounded-full" : "z-20 fixed top-0 left-0 right-0 flex items-center justify-between flex-wrap bg-teal-500 p-6 bg-opacity-0 ease-out duration-300"}>
            ...
            <div className={scrollNav === true ? "transition-all w-full hidden flex-grow lg:flex lg:items-center lg:w-auto text-xl text-teal-200" : "transition-all w-full hidden lg:flex lg:items-center lg:w-auto text-xl text-black"}>
                <div className="">
                    <a href="#responsive-header" className={scrollNav === true ? "block mt-4 lg:inline-block lg:mt-0 hover:text-white mr-10" : "block mt-4 lg:inline-block lg:mt-0 hover:text-gray-300 mr-10"}>
                      Docs
                    </a>
                    <a href="#responsive-header" className={scrollNav === true ? "block mt-4 lg:inline-block lg:mt-0 hover:text-white mr-10" : "block mt-4 lg:inline-block lg:mt-0 hover:text-gray-300 mr-10"}>
                      Examples
